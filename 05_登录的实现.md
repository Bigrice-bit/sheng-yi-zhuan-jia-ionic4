...
```bash
ionic g class shared/ajaxResult
```
...
```bash
ionic g interface shared/ajaxResult
```
...
参考之前的任务，创建找回密码组件。记得修改app.module.ts文件，引入ForgotPasswordPage并设置`@NgModule`元数据中的`entryComponents`属性。参考之前的任务实现找回密码的页面跳转。
找回密码导航栏的实现。  
...
在shared.module.ts文件中，在declarations属性中添加CopyrightComponent，然后添加exports属性。  *src\app\shared\components\shared.mudule.ts*
...
5. 在注册页中使用CopyrightComponent。在imports属性中添加SharedModule。ShareModule中已经导入了CommonModule、FormsModule和IonicModule，所以HomeModule中可以删除相应的模块。  
