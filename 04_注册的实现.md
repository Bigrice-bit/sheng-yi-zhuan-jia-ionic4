...
```html
<!-- 为ion-button元素添加href属性 -->
<ion-button color="primary" fill="outline" expand="block" routerLink="signup">注册</ion-button>
```

测试前在浏览器中把本地存储中App的isLaunch属性的值改为false。
...
根据需求请自行实现`onSubmitPhone`方法。直接点击下一步时发现页面没有给出错误提示，可以考虑为组件类添加一个submited属性，用来表示表单是否提交过。初始值为false，在onSignupPhone方法中把submited的值改为true。
